...
Mandatory shared computation boundary (user clarification, 2026-09-07): every optical trace and analysis must call the formal product's shared Core. Laboratory isolation applies to features, search orchestration, processes, and data, never a separate optical engine. Missing reusable capabilities must be implemented and verified in the formal shared layer, with no laboratory formula copies or approximation fallbacks. P1 already calls Core for real/paraxial tracing, but its local intercept-RMS summary must be moved behind the formal shared computation interface and checked for identical snapshot/settings behavior before P2 expansion. This reconciliation is planned, not implemented. Independent analytical/Snell code remains test-only and cannot supply runtime results. See proposal section 7 for the source audit and implementation order; this clarification changed documentation only and did not rerun tests.
