...
| 当前验证记录 | 持续维护 | 最近一次正式产品完整基线为 2026-09-03 的构建 `0` 错误及主测试 `1006/1006`；构建仅有 NuGet 漏洞数据源 SSL 警告。独立实验室最近一次完整测试为 `21/21`，使用独立解决方案与基线 |
...
- 验证记录：实际 `[MS-L7]` 的 103 行顺序、兼容参数槽位、光线操作数映射、常见操作数束、描述符驱动编辑往返、ZOS-API MFE golden 和条件控制流探针通过定向测试；2026-09-03 全量主测试 `1006/1006` 通过。
