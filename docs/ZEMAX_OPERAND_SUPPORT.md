...
当前状态复核：2026-08-04。本文是顺序模式操作数的目标规范与验收矩阵；“必须支持”不等于当前版本已经实现，实际完成度以文末“当前基线与差距”为准。代码、文件字段和产品专有名词保留原始英文标识。本轮分析目录与 UI 清理不改变操作数支持范围。
...
截至 2026-08-04，`MeritFunctionCatalog` 有 51 个 Workbench 代码或兼容记录类型，仍未满足 333 个顺序模式操作数的完整目标。ZMX 导入器已修正参考镜头 `[MS-L7](10X大NA大视场).ZMX` 的明显漏行：该文件评价函数区的 103 行现在按源顺序全部进入内存；其中 63 行 `TRAR` 作为现有可执行光线像差操作数导入，`TTHI`、`CTGT`、`PMAG`、`DIVI`、`REAR`、`DIMX`、`PETZ`、`MXEG` 和 `SINE` 按 ZMX 的 `Int1`、`Int2`、`Data1`–`Data4`、`Target`、`Weight` 槽位原样保存为禁用只读记录。禁用只读只表示往返兼容，不表示已经具备 Zemax 等价计算。

本次修正消除了参考文件中已知操作数被静默丢弃的问题，但没有把“参考文件可无损显示”扩大宣称为“完整 Zemax 评价函数支持”。其他尚未注册的顺序模式代码仍属于待实现范围，导入诊断和 333 项描述符仍需按下述实施顺序完成。

尤其需要消除以下技术债：

- 用 `Wavelength` 保存某些操作数的第二整数槽位；
- 对所有类型套用同一表面/视场/波长校验；
- 参考文件之外的未注册代码仍可能被导入器忽略，其他入口调用 `CanonicalType` 时仍会规范化为 `BLNK`；
- 未实现操作数返回零或只读注释；
...
1. **注册表与快照模型**：建立 333 项描述符；引入通用参数槽位和类型化验证；保证旧 schema 安全迁移。
