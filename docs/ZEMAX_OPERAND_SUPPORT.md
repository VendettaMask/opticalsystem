...
当前状态复核：2026-08-02。本文是顺序模式操作数的目标规范与验收矩阵；“必须支持”不等于当前版本已经实现，实际完成度以文末“当前基线与差距”为准。代码、文件字段和产品专有名词保留原始英文标识。
...
截至本文建立时，`MeritFunctionCatalog` 只有 42 个 Workbench 代码或别名，ZMX 导入器仅显式解析少量可执行操作数，并把部分 Zemax 指令/约束保存为禁用记录。因此当前版本尚未满足本规范。
