...
- 2026-08-28变更前历史基线为正式产品严格构建 `0` 警告、`0` 错误、全量回归 `837/837`；该数量已经由下文 2026-09-02 的 `1005/1005` 完整基线取代，不作为当前结果；
...
- 2026-09-02 常见 Zemax 顺序评价函数操作数束扩展、实机目录校准、描述符驱动编辑往返和 `[MS-L7]` MFE golden 接入后，正式主测试 `1005/1005` 通过，解决方案构建 `0` 错误；构建仅有 NuGet 漏洞数据源 SSL 警告。目录为 383 个顺序兼容代码、114 个已连接计算引擎的代码；103 行源哈希/顺序和 400 余个活动参数槽已锁定，12 个代表行通过数值对照，并修正 `PETZ` 符号、`TTHI/TGTH` 端点包含语义以及 `GOTO/ENDX/OOFF/SKIN/SKIS/USYM` 控制流；条件跳转对称分支另经本机 ZOS-API 贡献值实测；完整 Zemax 等价仍需继续收敛。
