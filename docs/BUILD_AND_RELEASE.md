...
- 2026-08-28变更前历史基线为正式产品严格构建 `0` 警告、`0` 错误、全量回归 `837/837`；该数量已经由下文 2026-09-02 的 `998/998` 完整基线取代，不作为当前结果；
...
- 2026-09-02 常见 Zemax 顺序评价函数操作数束扩展、实机目录校准和描述符驱动编辑往返完成后，正式主测试 `998/998` 通过，解决方案构建 `0` 错误；构建仅有 NuGet 漏洞数据源 SSL 警告。目录为 383 个顺序兼容代码、108 个已连接计算引擎的代码；RMS、Moore-Elliott、厚度、行约束和 `DIMX/EFNO` 参数语义已校正，桌面编辑保存不再用固定友好字段覆盖六个原始槽位；完整 Zemax 等价仍需逐项 ZOS-API golden 对照。
