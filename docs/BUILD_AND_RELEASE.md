...
- 2026-09-02 常见 Zemax 顺序评价函数操作数束扩展后，正式解决方案默认输出构建为 `0` 警告、`0` 错误；正式主测试 `997/997` 通过。新增覆盖包括 109 个已连接计算引擎的 Zemax 顺序操作数、常见厚度/边厚/曲率/圆锥/半口径、`WLEN/INDX`、若干一阶量、ZMX 导入槽位映射、评价函数贡献和兼容行失败语义；完整 Zemax 等价仍需 ZOS-API golden 对照。
