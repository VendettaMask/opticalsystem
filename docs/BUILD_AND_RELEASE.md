...
- 2026-08-28变更前历史基线为正式产品严格构建 `0` 警告、`0` 错误、全量回归 `837/837`；该数量已经由下文 2026-09-02 的 `997/997` 完整基线取代，不作为当前结果；
...
- 2026-09-02 常见 Zemax 顺序评价函数操作数束扩展后，正式解决方案默认输出构建为 `0` 警告、`0` 错误；正式主测试 `997/997` 通过。随后经本机 OpticStudio 2026 R1 实跑复核，目录修正为 383 个顺序兼容代码、108 个已连接计算引擎的代码，并校正 RMS、Moore-Elliott、厚度、行约束和 `DIMX/EFNO` 参数语义；完整 Zemax 等价仍需逐项 ZOS-API golden 对照。
