...
- 2026-08-28变更前历史基线为正式产品严格构建 `0` 警告、`0` 错误、全量回归 `837/837`；本次按用户要求不跑全量，新增回归后不得沿用该数量作为当前基线；
...
- CI 现于 Linux、macOS、Windows 同时恢复、构建和测试正式解决方案与独立 Initial Structure Lab；仓库质量任务分别验证两套解决方案格式。固定种子格式模糊回归覆盖 STAROPT、STARMESH、STARRDB 和 ZMX，当前定向结果 `6/6`。
