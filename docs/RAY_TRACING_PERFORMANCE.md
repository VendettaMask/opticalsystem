...
截至 2026-08-24，仓库当前记录 `675` 项回归测试，最近一次全量基线为 `663/663`。后续 GUI、fan 布局、报告菜单、分析目录清理、入射角、Zemax 评价函数导入以及架构/可访问性/响应式布局定向契约不构成新的性能测量，也不改变本文追迹性能基线；独立畸变入口退场没有删除组合分析实际使用的底层计算。
