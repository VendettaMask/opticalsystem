...
截至 2026-08-25，正式产品测试项目包含 `747` 项回归测试，最近一次完整基线仍为变更前的 `739/739`；本轮相关定向子集为 `34/34`，未运行正式产品全量测试。独立智能初始结构实验室定向测试为 `7/7`，不构成新的追迹性能测量。GUI、多配置状态、保存队列、几何 MTF 强度加权和响应式布局改动不改变本文追迹性能基线。
