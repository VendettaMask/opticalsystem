...
- `[MS-L7]` 评价函数导入与编辑往返定向测试继续通过；当前 111 个 Zemax 顺序操作数已有定义级可执行路径，覆盖 `TRAR`、`TTHI/TGTH`、`REAR/RANG`、基础数学与行约束、常见厚度/边厚/曲率/圆锥/半口径、`WLEN/INDX`、若干一阶量以及 `CTGT`、`PMAG`、`PETZ`、`MXEG` 和 `GOTO/ENDX/OOFF`。注册表按本机 2026 R1 实测扩展到 383 个顺序兼容代码；`DIMX` 等尚未完整实现语义的兼容操作数仍为禁用只读记录。新增执行路径仍需 Zemax/ZOS-API golden 对照后才能标为完整兼容。
