...
- `[MS-L7]` 评价函数导入与编辑往返定向测试继续通过；当前 109 个 Zemax 顺序操作数已有定义级可执行路径，覆盖 `TRAR`、`TTHI/TGTH`、`REAR/RANG`、基础数学与行约束、常见厚度/边厚/曲率/圆锥/半口径、`WLEN/INDX`、若干一阶量以及 `CTGT`、`PMAG`、`DIMX`、`PETZ`、`MXEG`。其余尚未实现语义的兼容操作数仍为禁用只读记录；新增执行路径仍需 Zemax/ZOS-API golden 对照后才能标为完整兼容。
