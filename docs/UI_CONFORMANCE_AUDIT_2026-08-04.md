...
本审计列出的 P1/P2 工程问题已完成基础整改，并由公共资源、公共组件和定向契约保护。2026-09-01 新增像素风格后，主题资源、对比度、图标包、Chrome、运行时切换和架构边界测试覆盖明亮、暗夜、异世界、像素风格四个具体主题；Headless 测试验证连续切换不会改变受测内容边界，系统选项按代理解析实际视觉主题，对话框装饰不会把 `ScrollViewer` 重复挂接到逻辑树。公司品牌、启动页、物理波长/光线色、工程图和导出版式为明确例外；后续新增页面仍须接受多 DPI、四主题和窄 Dock 的发布前视觉复核，这属于持续质量门禁，不作为自动像素验收。
