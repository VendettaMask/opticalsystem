...
- Extend Python Optiland JSON interoperability beyond the current validated sequential subset to the remaining freeforms, Python-preserved coating models, BSDFs, phase models, pickups, solves, telecentric systems, and polarization.
...
As of 2026-07-17, the solution builds with zero warnings and all `195/195` tests pass. Coverage includes finite structured plots for every catalog entry, generated analysis parameter settings, Python golden comparisons, tracing, serialization, optimization, tolerancing, plugins, visualization, and file formats.
