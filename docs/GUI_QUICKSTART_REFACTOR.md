...
截至 2026-08-25，正式产品测试项目包含 `747` 项回归测试，最近一次完整基线仍为变更前的 `739/739`。本轮相关状态、保存、MTF、可访问性和响应式子集为 `34/34`，未运行正式产品全量测试；新增覆盖多配置结构同步与撤销、保存请求顺序、损坏设置隔离、图标自动化名称和几何 MTF 强度/无数据边界。独立智能初始结构实验室定向测试为 `7/7`，不并入 GUI 重构基线。
