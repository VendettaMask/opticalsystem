...
分析结果统一提供“绘图 / 数据 / 文本”页。绘图支持指针中心缩放、拖动平移、双击重置和最近采样悬停。光线像差图默认启用“方形子图”，让每个 X/Y fan 面板尽量接近正方形；其它可伸缩多子图默认关闭但可手动启用，固定矩阵布局不受该选项影响。
...
截至 2026-08-03，仓库包含 `663` 项回归测试，当前全量基线为 `663/663`。窗口布局测试验证平铺/层叠回收浮动页并进入主文档区 MDI，合并回收浮动页并恢复标签模式；Ray Fan 方形面板契约验证光线像差图默认接近正方形。
