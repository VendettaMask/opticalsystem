...
2026-09-07：共享 Core 增加显式圆孔径余量诊断，复用正式表面追迹与光斑统计；未截光诊断不参与物理通光和最终验收，也不进入普通追迹缓存。独立初始结构实验室只据此组装搜索残差，没有增加光学引擎或产品反向依赖，见 [P5 孔径修复边界](INITIAL_STRUCTURE_P5_APERTURE_2026-09-07.md)。

2026-09-08：实验室 Engine 的 `Optimization` 增加 [S1 通用信赖域最小二乘](INITIAL_STRUCTURE_S1_SOLVER_2026-09-08.md)，输入只有有界变量和固定残差回调，拒绝步复用差分模型和 QR 解。该模块不调用光学类型或外部进程，没有新增包或产品反向依赖；尚未替换 v4 光学求解流程。耦合总长约束和物理可行性恢复属于待实施的 S2，不能用独立边界投影替代。

Huygens MTF 后处理现在明确区分普通/视场分析的 2N 零填充与离焦分析的 N 变换。
兼容预设在端点跨度上自然三次取值；Core 的一般 DFT 周期和线性默认保留。
原生 PSF 重建测试只依赖冻结 validation 资料，没有向 src 引入反向依赖。
详见 [后处理修复及仍有误差的传播边界](ZEMAX_HUYGENS_REPAIR_2026-09-06.md)。

2026-09-06 后续修复：几何/扩展源能量的捕获输出约定与 Core 通用 CDF 默认分离；几何线/边缘扩散修正网格和半区间积分。Contrast Loss 保留产品平均 OPD 指示器，另发布结构化原光瞳相位用于原生数组对照，辅助序列不作为摘要标量显示。依赖方向不变，见 [能量与相位边界](ZEMAX_ENERGY_REPAIR_2026-09-06.md)。

2026-09-06 全分析比较扩展：独立工具现在提供 52 项显式数值契约和 12 项点列/图像能力审计，原始证据置于 `validation/zemax/2026-r1`。依赖仍是 tools → Application/Core；src 不访问 ZOS-API、比较工具或测试资产。RMS 角向采样为显式选项，Core 一般默认值保留；几何、衍射、Jones 和报告路径修复及未解决的物理定义差异见 [扩展记录](ZEMAX_ANALYSIS_EXPANSION_2026-09-06.md)。

材料快照直接保存已解析目录玻璃的实际公式、系数与元数据，防止跨目录同名材料替换。分析工厂通过镜头瞄准设置驱动相关光线与衍射尺度；FFT PSF 的产品采样预设与 Core 一般默认分离。2026-09-06 的修复、数值证据与文件兼容性见 [数值修复记录](NUMERICAL_REPAIR_2026-09-06.md)。

Optical System Design 是纯 C#/.NET 计算系统，不包含 Python Optiland 运行后端。产品依赖方向为 App → Application → Core；App 不直接引用 Core，测试可以单向引用产品和 validation/history 的冻结资料。通用 INumericBackend、ManagedCpuBackend、CPU/SIMD 路径和后续 GPU 扩展边界保留。

## 核心对象模型

独立 Zemax 验证工具依赖方向为 `ZemaxComparison -> Application/Core`，没有生产项目反向引用。Application 的 `BuildAnalysisData` 和原 `BuildAnalysisView` 共用同一规范分析工厂；前者保留未舍入的类型化数据，后者继续生成 UI。ZOS-API 仅在验证目录的 C# Framework 工作进程中加载，单项超时以 Windows job 隔离。详见 [验证工具边界](ZEMAX_COMPARISON_TOOL.md)。

2026-09-05 数值与结果契约修复见 [项目修复报告](PROJECT_REPAIR_2026-09-05.md)。分析结果以 `AnalysisOutcome` 发布成功、不可计算或不适用状态及原因，并通过 Runtime/DTO 保留；状态页不可统计为数值计算成功。`NormalizedField` 是独立的无量纲轴类型，DTO 映射不依赖不同程序集枚举的相同序号。光斑显示光线与含波长权重的统计视图分开，质心和 RMS 共用统计权重；MTF 在共同物理坐标/频率中合成复数 OTF。串行公差和工作线程共用显式 optic 参数的变量工厂。
...
未知顺序几何由 `OpaqueGeometryPayload` 保存完整组件类型、数值、文本和递归子组件；其 Sag、交点和法线接口始终抛出确定错误。`OpticCapabilityPreflight` 是实光线追迹、全部公共旁轴/一阶入口、分析、优化、公差、顺序转非序列、二维/三维布局和导出的统一阻断入口，错误包含面号、原始类型及原因。STAROPT和原生光学快照可无损往返 opaque 数据；STEP、ZMX、SEQ、LEN和制造图纸等有损目标默认禁止输出。
...
优化由变量、操作数、缩放器和优化器组成。公开算法名称严格对应当前实现：阻尼最小二乘、Nelder-Mead、坐标模式搜索、动量梯度下降和贪心随机扰动。`LM / DLS`、`Least Squares` 和 `Orthogonal Descent` 只作为真实同义实现的带警告兼容别名；BFGS、L-BFGS-B、COBYLA、Powell、差分进化、双重退火和盆地跳跃在独立实现完成前明确返回不支持。结果记录实际算法、版本、停止原因、函数评价次数、梯度范数（适用时）和随机种子（适用时）。所有规范优化器和应用服务统一接受 `1..1000` 次迭代，零、负数和超限值在评价前明确失败，不再静默夹取或产生零迭代伪成功。优化变量、缩放器、操作数和向量入口都执行严格有限数和维度校验；上下界非法、步长非正、缩放正反变换非有限、向量长度不匹配或操作数返回 `NaN`/无穷都会立即失败。变量向量按事务提交，任一 setter 或操作数失败都会恢复原向量；若恢复本身失败，聚合异常同时保留原始失败和恢复失败。优化运行以包含全部配置和断开链接的完整文档为事务边界：取消、求解异常或最终自动半口径刷新失败都会恢复初始状态且不增加撤销记录；只有刷新成功后才发布优化变更。文档生命周期令牌使用独立同步门，取消不等待长期持有的光学模型锁。ZMX 评价函数导入保持源行顺序；`ZemaxOperandRegistry` 按本机 OpticStudio 2026 R1 实测注册 383 个顺序兼容代码，快照为每行独立保存原始整数/数据槽位及 `CompatibilityOnly` 状态，描述符记录槽位引用类型。注册和无损往返不等于完整计算支持；当前 124 个 Zemax 代码连接已有计算路径，其中 `TTHI/TGTH`、`REAR/RANG`、基础数学与行约束（含 `DIVB/PROB/OSUM/QSUM/EQUA` 定义级语义）、常见厚度/边厚/曲率/圆锥/半口径、`WLEN/INDX`、`MNIN/MXIN/MNAB/MXAB`、`POWR` 和若干一阶量已进入定义级可执行路径，`MNIN/MXIN/MNAB/MXAB` 只约束范围内玻璃 Nd/Vd，`POWR` 仅对标准折射面计算 `(n_after − n_before) / Radius`；`TTGT/TTLT/TTVA` 按指定表面到下一表面的边缘总厚度计算而不是系统总长；`GOTO/ENDX/OOFF/SKIN/SKIS/USYM` 已接入有序控制流并通过本机 ZOS-API 对称分支实测；`DIMX` 因 Field/Absolute 语义未完成而保持兼容只读。新增路径仍需 Zemax/ZOS-API golden 对照后才能标为完整兼容。其余未完成目标代码禁用兼容保留；禁用兼容行不参与计算；未知类型或被错误启用的兼容行返回明确错误，不会转成 `BLNK` 或成功零值。公差复用变量和操作数模型，支持扰动、采样、补偿、双侧灵敏度及按种子确定的 Monte Carlo。外层并行拥有执行权时，内层追迹会抑制嵌套并行。
