...
GUI analyses share a bounded ray-trace cache across independent snapshots of the same workspace revision. Cache identity uses the revision, numeric backend, exact input-ray state (therefore field, wavelength, pupil sample, intensity, OPD, polarization, and normalization), resolved retained surface indices, and result-affecting request options. `FinalOnly` and `SelectedSurfaces` may reuse one entry when both resolve to the same surface and options. The cache never keys on an analysis name or localized label. A new workspace revision clears retained entries, and any trace-relevant mutation of a worker snapshot detaches that snapshot from the shared cache before another trace. Requests that record `SurfaceGroup.RecordedTrace` bypass the shared cache because recording is an explicit side effect. Deterministic pupil sampling plans are cached separately by sampling mode, count, seed, and mode-specific parameters.

Interaction results distinguish transmission, ordinary reflection, and total internal reflection. The actual `RayInteractionKind` is retained on every successful trace sample instead of being discarded after the next ray state is formed. Only transmission advances the current material; reflection and total internal reflection retain the incident medium. Directions emitted by a thin lens are normalized before the next surface, keeping geometric path, OPL, and absorption based on physical distance.
...
Visualization layout is generated in core before Avalonia rendering. `Layout2DBuilder` samples each surface with `Sag(0, y)` in the YZ projection, groups lens surfaces by material transitions, extends smaller-aperture lens faces to the group's maximum extent before closing the 2D body, and builds 3D surface rims/meridians from the same geometry. Ray paths are emitted as directed segments carrying normalized propagation direction, source/target surface numbers, typed surface interaction (`Refractive`, `Reflective`, `Diffractive`, `ThinLens`, or `Phase`), and the actual propagation result (`Incident`, `Transmitted`, `Reflected`, or `TotalInternalReflection`). These fields survive the application Scene DTO boundary. The Avalonia viewer draws and arrows each segment from that direction metadata, never from assumed positive-Z order or localized labels; clipped or vignetted rays still stop at the recorded failure point.
...
`PanelManager` delegates ownership and drag/drop behavior to Dock instead of creating Avalonia `Window` objects or transferring tabs manually. It provides bulk dock, float, tile, cascade, close, lock, and layout commands. Floating is an explicit operation: tile and cascade only reposition the current `IRootDock.Windows` entries and must not float docked documents or create additional host windows. `MainWindow` owns application commands, file dialogs, appearance, and top-level lifecycle.
...
The analysis panel consumes structured DTO data. Its lifecycle, parameter editor, result presentation, plot layout, and export logic are split into focused files. `AnalysisViewDto.PresentationKind` is the stable, typed contract used to select specialized result controls; localized names, visible titles, and imported labels are display-only data and must never participate in that dispatch. Every numerical series also carries `AnalysisAxisQuantity` and `AnalysisAxisUnit` for X, Y, and optional value/color data. Plot range normalization, hover formatting, compatible mm/µm/nm and degree/radian conversion, and CSV export use those typed fields; legacy axis-label text is sanitized only for display and is never used to infer a unit. Metric, graph, and report views are built without parsing display strings. Each analysis document owns one settings instance and bottom-aligned Plot/Data/Text result views. Heavy analyses do not rerun on ordinary edits: the event revision marks the result stale, and the icon-only synchronization action starts a cancellable snapshot calculation. Instance ID, generation, and source revision must all match before a result is accepted.
