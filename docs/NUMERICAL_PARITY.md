...
Python Optiland 0.5.8 does not expose Zemax's real-image-height field type. The Workbench implements it as a documented Zemax extension: a damped two-dimensional solve adjusts the infinite-conjugate object angle or finite-conjugate object coordinate until the primary-wavelength chief ray reaches the requested local image-surface X/Y coordinate. ZMX field rows keep their declared one-based order even when X/Y coordinates are duplicated, so field weights, comments, merit-function references, and multi-configuration references keep Zemax numbering. Plain ZMX imports activate configuration 1 when no explicit active configuration can be recovered, rather than guessing from operand proximity to the base prescription. ZMX `APMN`/`APMX` configuration operands feed the imported surface aperture state; positive `APMN` values create an `AnnularAperture`, so center obscurations vignette traced rays rather than being reduced to semi-diameter metadata. Distortion temporarily converts real image height to field angle or object height, while image simulation temporarily uses paraxial image height. Field curvature and distortion then scan a NumPoints half-fan along the selected `+x/-x/+y/-y` direction, switch tangential/sagittal planes for X scans, and either ignore or apply vignetting factors without mutating the source optic. All field types use the same Python-compatible maximum-radial-field normalization.
...
Tests compare deterministic output at its native level: traced samples and fitted sphere parameters, curve points, wavefront/Zernike values, heatmap and PSF pixels where the fixture remains the semantic authority, MTF values, and distortion coordinates. FFT/MMDFT/Huygens diffraction PSFs additionally lock the Zemax/physical rule that traced power `Intensity` becomes coherent field amplitude through `sqrt(Intensity)`, so older direct-intensity fixture pixels are treated as reference-scale checks rather than exact authority. Sampled MTF regressions require true zero-frequency normalization to 1, the same continuous wavefront model on both sides of the pupil autocorrelation, and Zemax pupil-space Tangential/Sagittal labeling while leaving FFT MTF and Contrast Loss Map directions unchanged. Huygens regressions additionally require exact adjacent-point Image Delta and the local image-surface normal for tilted and curved detectors. RMS scan regressions require the Show Diffraction Limit switch to add a dashed horizontal series for Field/Focus and a wavelength-dependent spot reference curve for Wavelength, while preserving Zemax's approximate `1.22 × F/# × wavelength`, `0.072 waves`, and `0.8 Strehl` thresholds. Image-simulation regressions validate the Zemax workflow invariants—selectable None/Geometric/Diffraction modes, black guard bands, Field Height mapping, relative illumination, distortion/lateral color, and severe-aberration fallback—rather than preserving obsolete fixed-FFT RGB pixels. Presentation tests verify pane order, labels, line pairing, marker/style choices, zero lines, aspect, legends, limits, and colorbars. The normal analysis tolerance is `2e-8 * max(1, abs(expected))`.
...
RMS-vs-field Spot regressions require `Field Density + 1` samples from zero through the maximum defined field magnitude along the selected orientation; discrete Field Editor rows define the system range but are not the scan samples. Wavefront RMS regressions keep chief and centroid references distinct: chief removes weighted piston only, while centroid removes the weighted best-fit piston and both pupil tilts.
...
The validated repository baseline is `621/621` passing tests as of 2026-08-02.

The default accuracy authority is the committed Zemax OpticStudio 2026 R1 `123456.ZMX` baseline; the Optiland 0.5.8 Cooke/Tessar suite remains an auxiliary regression. The detailed accuracy, page-contract, and tracked Zemax-baseline results are recorded in [Accuracy validation](ACCURACY_VALIDATION_2026-07-31.md). The current 30-analysis comparison contains 26 highly consistent, 4 close, and 0 significantly different mappings, with 2 additional non-equivalent reference-sphere mappings excluded from accuracy classification. Captured values such as sampling, field/wavelength selection, focus range, and point count apply only to that file and capture; they are passed explicitly by parity tests and are not universal Zemax defaults. The geometric encircled-energy product path follows the captured polychromatic, centroid-referenced, diffraction-limit-scaled definition; exact Optiland 0.5.8 point-for-point behavior remains available only through the explicit compatibility path used by the auxiliary regression. For the `123456.ZMX` comparison, Diffraction Encircled Energy uses the captured 401-point output together with stretched-pupil FFT image sampling, finite PSF-pixel area integration, and a no-OPD diffraction-limit automatic range; its six-curve median NRMSE is 1.229% and worst NRMSE is 1.478%, without an empirical scale or offset. The captured Huygens Through Focus MTF case uses 20 cycles/mm, a 64×64 pupil/32×32 image, automatic `λF/√nₚ` Image Delta, wavelength-weighted incoherent PSF, and 101 output points. Huygens MTF vs Field evaluates the OTF directly at the six requested spatial frequencies rather than linearly interpolating below the first coarse FFT bin. The captured Huygens MTF case uses saved 32×32 pupil/image sampling, automatic Image Delta, incoherently combined polychromatic PSF, and `2N−1` frequency support; its 10-curve median NRMSE is 0.194% and worst NRMSE is 1.696%, with no empirical scale or offset.
