...
The validated repository baseline on 2026-07-31 is a zero-warning solution build with `592/592` passing tests. The detailed 2026-07-31 accuracy, page-contract, and tracked Zemax-baseline results are recorded in [Accuracy validation](ACCURACY_VALIDATION_2026-07-31.md). The current comparison reruns all 69 Workbench analyses, preserves 69 raw result files and 69 Workbench/Zemax screenshot pairs, and numerically maps 32 analyses: 12 are highly consistent, 5 are close, and 15 remain significantly different. It also records a 6.80× Huygens Through Focus MTF runtime regression. The validation includes the Huygens image-surface normal and Image Delta corrections, selectable Zemax-style image simulation, distortion and RMS reference-series regressions, compact spot-diagram layout, optional square multi-pane layout, current glass-catalog priority persistence, merit-operand row-color coverage, and palette-backed light/dark/异世界 plot and scene theme resource coverage.
