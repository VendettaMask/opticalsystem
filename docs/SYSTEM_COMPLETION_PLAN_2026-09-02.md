...
Huygens 后处理修复后，MS-L7 重新执行全部 72 项，总计 44 Pass、6 Close、2 Difference。详见 [完整证据与未解决项](ZEMAX_HUYGENS_REPAIR_2026-09-06.md)。此前几何/扩展源能量和线边缘扩散已经修复；相位数组与 GUI 平均 OPD 分开核对。未完成工作继续保留：

- Huygens 频率/视场与离焦的变换大小、频率跨度和插值已经独立验证并修复；见 [后处理证据](ZEMAX_HUYGENS_REPAIR_2026-09-06.md)。剩余工作是验证 PSF 的振幅权重、光瞳积分与传播模型，以及衍射圈入能量的完整积分/显示链条，不能以后处理通过代替全链条通过。视场 MTF 的局部退步继续记录。Contrast Loss 的 GUI 平均 OPD 指示器未包含在原生数组 Pass 中，需另建可核验的 GUI/指示器契约；不得拟合偏移、峰值或放宽容差。
- 补齐 Image Simulation、Geometric Image、Geometric Bitmap、Light Source、Extended Diffraction 的共同源、光谱、探测器、单位与辐射量契约。已证实前两项可分别导出 BMP/数值网格，不能归为 API 不支持。
- 渐晕、Foucault、部分相干需要先明确产品物理定义或实现相应传播模型，不能将现有系数图、阴影近似或强度混合直接宣称等价。
- 四项点列布局保留实际无数值通道证据；后续如增加可审计的逐光线匹配，必须明确其与布局图本身的范围差别。

这些是当前未解决能力，不是已完成精度覆盖，也不引入任何 Python Optiland 运行或未来互操作计划。

## 文档状态

- 复核日期：2026-09-05
- 历史验证：2026-09-06 Huygens MTF 后处理修复后的正式解决方案 Release 默认输出构建 `0` 警告、`0` 错误，完整主测试 `1233/1233`、工具测试 `104/104`，合计 `1337` 项通过，零失败、零跳过；锁定依赖从本地缓存还原，格式检查和 git diff --check 通过。MS-L7 重新执行全部 72 项，结果为 44 Pass、6 Close、2 Difference、17 Incomparable、3 Skipped，0 执行错误；主基准独立复验三项 Huygens MTF 均 Pass。新增 14 项回归包含原生 PSF 后处理重建，不能代替全链条精度结论。视场 MTF 的一处分量由 Pass 变为 Close，局部退步及未解决误差监控预算已明确记录，正式数值容差不变。未执行在线漏洞审计、独立实验室、旧外部报告工具、GUI 截图或安装包实测。该阶段证据见 `ZEMAX_HUYGENS_REPAIR_2026-09-06.md`，以下更早的移除审计仅作历史记录。详见 [移除审计与验证](PYTHON_OPTILAND_REMOVAL.md)。
- 范围：正式 Workbench、Zemax 和原生文件格式、非序列模式、优化与公差、Initial Structure Lab、UI 产品表达
- 性质：实施计划，不是当前完成能力清单
...
3. 与 Zemax 的对齐必须区分导入可见、原样保存、可执行计算和数值等价；
4. 每个完成项都要同步代码、测试、文档和验证结果。

## 总优先级

2026-09-05 首轮独立比较工具的 10 项适配器记录保留为历史证据。当前已扩展至 52 项数值契约，Fringe Zernike 等已进入实际捕获与回归；剩余 5 项图像适配、3 项物理定义差异、4 项 API 点列输出限制及 12 项 Close/Difference 数值结果见本文最新收口状态。工具仍不进入产品依赖，不将“已枚举”“可执行”或非回归测试通过视为“已完成数值对齐”。
...
Initial Structure Lab 是独立实验室功能，不属于正式产品能力。后续算法和里程碑已由 [2026-09-07 平板生成方案](INITIAL_STRUCTURE_FROM_FLAT_PROPOSAL.md)取代旧 L 系列计划：P0～P4 已完成，包括目录玻璃离散搜索、预算恢复和独立桌面工作流；P5 已进行困难规格修复、冻结发布验收与新处方 Zemax 对照，见 [P5 记录](INITIAL_STRUCTURE_P5_2026-09-07.md)。SQLite、CMA-ES、学习模型等旧方案候选技术不是本轮必需交付；跨平台人工 UI 验收仍未完成。正式入口只启动独立进程，光学计算始终共用正式 Core。
...
正式产品只使用 C#/.NET 计算，不包含 Python Optiland 后端或文件互操作，也不再规划恢复这些功能。通用 CPU/SIMD 数值抽象及未来 GPU 后端继续保留。Optiland 0.5.8 现有资料只保留为 validation/history 的冻结辅助历史回归，禁止新增对照；后续外部验证以已提交的 Zemax OpticStudio 2026 R1 基准为主，并明确镜头、设置、版本、单位、采样和容差。程序集及命名空间重命名单独立项。
