...
- 性质：实施计划，不是当前完成能力清单
- 当前仓库状态：计划文档已建立；Zemax 评价函数真实化已启动，本机 OpticStudio 2026 R1 实测目录已同步为 383 个顺序兼容代码，111 个操作数完成定义级可执行接入；未完成项主要来自明确的兼容边界、实验性模型、Zemax golden 对照缺口和路线图差距
...
4. **有序评价函数虚拟机**
   ...
   - 已支持 `GOTO` 前向跳转、`ENDX` 终止和 `OOFF` 惰性行；跳转目标必须位于当前行之后且不超过评价函数末行。
   - 后续支持 `SKIN`、`SKIS` 条件控制行。
