...
described below adds the 45 currently compatible open designs, producing the
106-entry packaged library. Microscope sources use a positive objective
...
```text
local-data/lens-library/originals/public-zmx/manifest.json
```
...
Every successful file is installed through the same single-file importer into
...
types not represented by the current geometry model.
