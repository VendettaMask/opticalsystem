...
| 明显差异 | 0 项 |
| 非等价映射排除 | 2 项 |
| .NET 回归测试 | 2026-09-03 正式解决方案构建 0 错误；正式主测试 1015/1015 通过。本轮构建无警告；独立实验室定向测试不参与精度结论 |
...
评价函数导入验证与上述 69 页分析精度基线相互独立。`[MS-L7]` 的 103 行评价函数顺序和源参数已由定向测试锁定；当前 124 个 Zemax 顺序操作数进入定义级可执行路径，覆盖 `TRAR`、`TTHI/TGTH`、`REAR/RANG`、基础数学与行约束（含 `DIVB/PROB/OSUM/QSUM/EQUA` 定义级语义）、常见厚度/边厚/曲率/圆锥/半口径、`WLEN/INDX`、`MNIN/MXIN/MNAB/MXAB`、`POWR`、若干一阶量以及 `CTGT`、`PMAG`、`PETZ`、`MXEG` 和 `GOTO/ENDX/OOFF/SKIN/SKIS/USYM` 控制行。`DIMX` 因尚未实现指定视场和绝对长度模式而降为兼容只读。仍禁用只读的兼容操作数以及尚未经过 Zemax golden 对照的新增执行路径，不能计入“高度一致”或“接近”的分析数值结论。
