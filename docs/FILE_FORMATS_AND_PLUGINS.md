...
当前容器版本为2、工程负载版本为3；顺序光学快照使用 schema 4 `OpticSnapshot`。负载保存：
...
- 独立非序列文档的波长、追迹默认值、类型化光源/几何/探测器对象、GUID引用关系，以及内容寻址的STL网格资产。

加载器除校验容器完整性外，还验证顺序主波长、有限数值、表面编号、组件布局，以及非序列对象类型、唯一GUID、引用图、网格资产哈希和集合上限。容器v1及工程负载v1/v2继续兼容；保存始终写容器v2和负载v3。构建在临时文档中完成，全部成功后才替换活动状态。`OpticSnapshot` schema 1–3 会先迁移到安全的 schema 4 状态。

非序列分析光线可另存为 `.starrdb`。该文件保存场景哈希、追迹设置和分块压缩光线树，通过统一路径筛选驱动路径分析、3D布局和探测器重建；它是可重新生成的分析结果，不嵌入STAROPT。详见[非序列第二阶段：杂散光基础链路](NONSEQUENTIAL_PHASE2_STRAY_LIGHT.md)。
...
当前 ZMX 顺序导入器明确拒绝 Zemax 非序列文件、未知表面类型、未支持的坐标断点顺序、经纬仪视场以及不可表示的环曲面项。有符号厚度按源值保留，不再把负厚度笼统视为非法。ZMX 不可靠保存 UI 活动配置，因此导入固定激活配置 1，同时保留全部配置。
