...
非序列分析光线可另存为 `.starrdb`。该文件保存场景哈希、追迹设置、随机种子、可选分裂模式和分块压缩光线树，通过统一路径筛选驱动路径分析、3D布局和探测器重建；它是可重新生成的分析结果，不嵌入STAROPT。未指定保存路径时，分析服务创建并管理临时数据库，以保证所有窗口读取同一次追迹。详见[非序列第二阶段：杂散光基础链路](NONSEQUENTIAL_PHASE2_STRAY_LIGHT.md)。
