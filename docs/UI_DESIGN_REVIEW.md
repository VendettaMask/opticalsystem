...
2026-09-01 像素风格主题：新增独立 `PixelTheme` 主题包，采用奶油白、天空蓝、亮蓝、珊瑚红、向日葵黄和草绿色的明亮 8-bit 色板。视觉复核后补齐主题专属 Fluent 控件资源：按钮为浅蓝硬边与无模糊硬阴影，输入框为奶油底深蓝边，折叠标题和表头使用亮蓝标题带，活动页签使用珊瑚红，列表/表格选择使用向日葵黄，复选状态使用草绿色；各状态不再全部复用单一强调色。卡片、控件框和对话框统一为零圆角硬边；Ribbon、工作区、视口和对话框装饰使用输入透明的代码矢量层绘制点阵、阶梯角标和像素明暗边。`PixelLucide` 保留全部现有语义图标映射，使用无抗锯齿边缘、方形线帽、直角连接和 1.35 倍描边形成像素轮廓，因此不引入新图标来源或授权。结构边框厚度、布局、文案、命令、物理光线颜色和分析语义保持不变；Headless 回归直接实例化 Fluent 按钮和输入框，验证运行时资源确实进入控件模板，而不只存在于主题字典中。
