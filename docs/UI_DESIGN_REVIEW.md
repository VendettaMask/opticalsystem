...
2026-08-04 分析设置浮层收敛：分析页设置面板不再使用顶部长条 `WrapPanel` 横向铺满窗口。普通分析参数、Zemax 风格参数和查看器设置统一为左上浮层卡片；卡片背景使用新增 `SettingsOverlaySurface` 半透明主题资源，文字和输入控件保持正常不透明；参数按两列字段排列，底部保留自动应用、应用、确定/取消、保存/载入/重置等操作区。该规则防止少量设置项被拉到窗口最右侧，也避免不同分析页在设置展开时出现完全不同的布局。
