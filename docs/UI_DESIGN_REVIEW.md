...
2026-08-04 卡片 chrome 收敛：`SettingsPanelChrome` 从设置面板专用样式扩展为普通 UI 卡片/控件框架的公共入口，固定卡片圆角 8、控件圆角 5，并集中普通卡片阴影。材料库列表与详情卡、查看器工具卡、公差操作数编辑卡、显示设置预览框、系统属性编辑卡、Ribbon 命令按钮和分析导出按钮已迁移到公共 token；镜头编辑器移除局部工具栏阴影。新增 `AppUiCardsUseSharedChromeTokens` 契约测试，普通 App 源码中只允许公共卡片阴影和 Ribbon 顶层 chrome 阴影，禁止再出现 6/7/9 这类分裂圆角。图例数据标记、Dock 文档 pill、Splash 装饰和导出版式继续作为明确例外保留。
...
2026-08-04 分析设置浮层收敛：分析页设置面板不再使用顶部长条 `WrapPanel` 横向铺满窗口，也不再作为 toolbar 内容把绘图区向下压缩。普通分析参数、Zemax 风格参数和查看器设置统一为左上浮层卡片；卡片覆盖在图形结果区上方，背景使用新增 `SettingsOverlaySurface` 半透明主题资源，文字和输入控件保持正常不透明；参数按两列字段排列，底部保留自动应用、应用、确定/取消、保存/载入/重置等操作区。该规则防止少量设置项被拉到窗口最右侧，也避免不同分析页在设置展开时出现完全不同的布局。
