...
2026-08-24 可靠性与窄窗口复核：图纸页工具条现可换行，设置/预览在 900 宽度以下上下重排；公差编辑器在 800 以下把表格与属性区上下排列，公差向导最小宽度降至 640 并把导航响应式移到上方；库存镜头匹配参数改为可换行字段。四个自绘交互控件继续提供命名 `AutomationPeer`、键盘平移/缩放/复位，并新增显式焦点边框；图纸缩放图标按钮发布自动化名称。设置 JSON 损坏时会隔离为带时间戳的备份并在状态栏报告，不再静默回退。重置或载入会销毁内容的布局前会单独保护未保存公差数据。以上均为已实现并有定向契约覆盖的行为；Ribbon 的固定高度和全部 Dock 页的更广泛窄宽度复核仍是后续工作。
