...
2026-08-04 分析结果底部样式统一：普通分析页的“绘图 / 数据 / 文本”页签改用与绘图正文一致的 11.5 字号、30 高度和统一内边距，避免 Fluent 默认页签在高 DPI 下显得过大。分析摘要、视场指标、右侧品牌与文件信息收敛为标题 14、正文 11.5、说明 10.5 的三级字阶，并分别绑定主题主文字、次文字和弱化文字颜色；摘要区高度由 126 缩为 108，右侧品牌/文件分隔固定在 66，状态栏指标列统一为 128 宽及相同字号、颜色和间距。分隔线与表面色继续使用主题资源，因此明亮、暗夜和异世界主题保持各自配色。
