...
2026-09-01 像素风格主题：新增独立 `PixelTheme` 主题包，采用奶油白、天空蓝、亮蓝、珊瑚红、向日葵黄和草绿色的明亮 8-bit 色板。卡片、控件框和对话框统一为零圆角硬边与无模糊像素阴影；Ribbon、工作区、视口和对话框装饰使用输入透明的代码矢量层绘制点阵、阶梯角标和像素明暗边。`PixelLucide` 保留全部现有语义图标映射，只把线帽、连接和描边改为方形像素轮廓，因此不引入新图标来源或授权。结构边框厚度、布局、文案、命令、物理光线颜色和分析语义保持不变。
