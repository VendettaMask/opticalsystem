...
| File format | `Serialization`, `FileIO` | Versioned `.staropt` project container with integrity checks, schema-3 semantic validation, transactional reconstruction, safe schema-1/2 migration, and multi-configuration persistence; legacy native JSON import, validated Python Optiland 0.5.8 JSON adapter subset, Optiland-compatible Zemax sequential import/export, plus SEQ/LEN common subset adapters |
