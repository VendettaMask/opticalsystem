...
- 镜头数据表右键所在行可执行“下插入、上插入、删除”，不再使用上方“添加 / 删除”按钮。物面与像面受保护，操作支持撤销/重做；菜单当前仅提供这三项。
- 曲率半径右侧标记区可设置“固定 / 变量 / 拾取”，代替原“R 变量”列；变量显示 `V`，拾取显示 `P`。拾取可指定前面的源面和曲率比例，详见[半径求解说明](docs/RADIUS_SOLVE_EDITOR.md)。
- 表格上方“表面 N 属性”横条可展开/收起属性，左右圆形箭头切换上一面/下一面并同步选中行。
...
Windows 安装包：双击根目录 `Build-Installer.cmd`（`Build-Exe.cmd` 等效），生成带中文安装向导的 `*-Setup.exe`，支持选择目录、快捷方式和卸载。输出位于 `artifacts/installers/`，包含 .NET 运行时；分发时只需提供 Setup EXE。首次构建会准备并校验官方 Inno Setup 编译器，安装包目前未签名。参数、便携版与验证范围见[构建与发布](docs/BUILD_AND_RELEASE.md#windows-一键生成带安装向导的-exe)。
