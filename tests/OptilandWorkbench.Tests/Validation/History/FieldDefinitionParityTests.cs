...
public sealed class FieldDefinitionParityTests
{
    ...
    [Fact]
    public void FieldDefinitionsMatchFrozenReferenceInitialAndFinalRealRays()
    {
        ...
        foreach (var expectedCase in root.GetProperty("cases").EnumerateArray())
        {
            ...
            var optic = name.StartsWith("finite_", StringComparison.Ordinal)
                ? FrozenHistoryFixture.FiniteSystem()
    ...
    [Fact]
    public void ParaxialImageHeightUnitChiefRaysMatchFrozenReference()
    {
        using var reference = LoadReference();
        var root = reference.RootElement;
    ...
    [Fact]
    public void NonImageFieldsMatchFrozenReferenceAndImageFieldsSatisfyTheLinearConjugates()
    {
        using var reference = LoadReference();
        var root = reference.RootElement;
        var hy = root.GetProperty("normalized_field").GetProperty("y").GetDouble();
    ...
    {
        var optic = FrozenHistoryFixture.FiniteSystem();
        optic.FieldDefinition = FieldDefinitionKind.RealImageHeight;
        optic.Fields.Clear();
        optic.Fields.Add(new FieldPoint { Label = "On axis" });
    ...
    [Fact]
    public void ViewerUsesRadialNormalizationForDiagonalFields()
    {
        var optic = Optic.CreateCookeTriplet();
        optic.Fields.Clear();
        optic.Fields.Add(new FieldPoint { Label = "On axis" });
    ...
    [Fact]
    public void NativeSnapshotRoundTripPreservesFieldContracts()
    {
        var optic = FrozenHistoryFixture.FiniteSystem();
    ...
    {
        var path = FrozenHistoryFixture.PathFor("optiland-0.5.8-field-definition-reference.json");
