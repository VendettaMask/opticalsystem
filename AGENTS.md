...
- The initial-structure laboratory must use the formal product's shared Core for all optical ray tracing and analyses, including paraxial quantities, material dispersion, spot metrics, wavefront, MTF, and layout ray data. Laboratory independence means separate features, search orchestration, process, and data, never a second optical engine. Laboratory code may assemble search residuals and compare returned results with targets, but must not implement or copy optical tracing/analysis formulas or use approximation fallbacks. Missing reusable capabilities must be added to the formal shared computation layer and verified there before laboratory use. Independent analytical/reference checks belong only in tests and must never supply runtime candidate evaluation, acceptance, or display results. Before continuing P2, reconcile the P1 laboratory-local intercept-RMS summary with this shared-analysis requirement; prior P1 test success does not establish that this reconciliation is complete.
...
- Product computation must stay pure C#/.NET: no Python, Python.NET, Optiland package, external Python process or service, or Python Optiland document interchange. Keep general numeric backends independent of external validation tools.
- Optiland 0.5.8 data under `validation/history` is frozen auxiliary historical regression evidence only. Never add or regenerate Optiland comparisons, and never reference history assets from product projects. Use committed Zemax OpticStudio 2026 R1 captures as the primary external numerical validation reference.
